...
LinkLuaModifier( "modifier_kassadin_riftwalk", "heroes/kassadin/modifier_kassadin_riftwalk.lua", LUA_MODIFIER_MOTION_NONE )
...
function kassadin_riftwalk:OnSpellStart()
	if IsServer() then
		...
		DebugDrawCircle( self:GetCaster():GetAbsOrigin(), Vector(0, 255, 0), 1.0, damage_radius, true, 3.0)
		...
		local units = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, damage_radius, ability_team_filter, ability_type_filter, ability_flag_filter, FIND_ANY_ORDER, false)
